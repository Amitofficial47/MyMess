"use client";

import { useState, useEffect } from "react";
import {
	Card,
	CardContent,
	CardHeader,
	CardTitle,
	CardDescription,
} from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { useData } from "@/contexts/data-provider";
import { useToast } from "@/hooks/use-toast";
import {
	Table,
	TableBody,
	TableCell,
	TableHead,
	TableHeader,
	TableRow,
} from "@/components/ui/table";
import { Badge } from "@/components/ui/badge";
import {
	Search,
	Hourglass,
	Loader2,
	Users,
	UserCheck,
	Building,
	ShieldCheck,
	Settings,
	Bell,
	FileUp,
	MessageSquare,
	Contact2,
	Shield,
	Utensils,
	CheckCircle,
} from "lucide-react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import * as z from "zod";
import {
	Form,
	FormControl,
	FormField,
	FormItem,
	FormMessage,
} from "@/components/ui/form";
import { useAuth } from "@/contexts/auth-provider";
import { Skeleton } from "@/components/ui/skeleton";
import Link from "next/link";
import { fetchApi } from "@/lib/api";
import type { MealSelection, AddonConsumption } from "@/lib/types";
import { Separator } from "@/components/ui/separator";

interface VerifiedMealData {
	meal: MealSelection & { studentName: string };
	addons: { name: string; quantity: number; price: number }[];
}

// Main component to switch between dashboards
export default function DashboardPage() {
	const { user, loading } = useAuth();

	if (loading || !user) {
		return (
			<div className="flex h-full items-center justify-center">
				<Loader2 className="h-8 w-8 animate-spin" />
			</div>
		);
	}

	if (user.role === "superadmin") return <SuperAdminDashboard />;
	if (user.role === "admin") return <AdminDashboard />;
	if (user.role === "messadmin") return <MessAdminDashboard />;

	return null;
}

// --- Super Admin Dashboard ---
function SuperAdminDashboard() {
	const [stats, setStats] = useState({
		studentCount: 0,
		adminCount: 0,
		messAdminCount: 0,
		hostelCount: 0,
	});
	const [isLoading, setIsLoading] = useState(true);

	useEffect(() => {
		async function fetchStats() {
			setIsLoading(true);
			try {
				const res = await fetchApi("/api/stats");
				if (res.ok) {
					const data = await res.json();
					setStats(data);
				} else {
					console.error("Failed to fetch stats: ", await res.text());
				}
			} catch (error) {
				console.error("Failed to fetch stats", error);
			} finally {
				setIsLoading(false);
			}
		}
		fetchStats();
	}, []);

	const quickActions = [
		{
			title: "Manage Admins",
			href: "/admin/manage-admins",
			icon: Shield,
			description: "Add or remove hostel admins.",
		},
		{
			title: "Manage Mess Admins",
			href: "/admin/manage-mess-admins",
			icon: Contact2,
			description: "Manage token verifiers.",
		},
		{
			title: "Edit Menu",
			href: "/admin/menu",
			icon: Settings,
			description: "Update the weekly meal plan.",
		},
		{
			title: "Send Notifications",
			href: "/admin/notifications",
			icon: Bell,
			description: "Broadcast announcements.",
		},
	];

	return (
		<div className="flex flex-col gap-8">
			<div className="text-center">
				<h1 className="text-2xl font-bold md:text-3xl lg:text-4xl">
					Super Admin Dashboard
				</h1>
				<p className="text-muted-foreground">
					System-wide overview and management.
				</p>
			</div>

			<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
				<OverviewStatCard
					title="Approved Students"
					value={stats.studentCount}
					icon={Users}
					isLoading={isLoading}
				/>
				<OverviewStatCard
					title="Hostel Admins"
					value={stats.adminCount}
					icon={Shield}
					isLoading={isLoading}
				/>
				<OverviewStatCard
					title="Mess Admins"
					value={stats.messAdminCount}
					icon={Contact2}
					isLoading={isLoading}
				/>
				<OverviewStatCard
					title="Managed Hostels"
					value={stats.hostelCount}
					icon={Building}
					isLoading={isLoading}
				/>
			</div>

			<div>
				<h2 className="text-xl font-bold md:text-2xl mb-4 text-center md:text-left">
					Quick Actions
				</h2>
				<div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
					{quickActions.map((action) => (
						<Link href={action.href} key={action.href} className="flex">
							<Card className="hover:bg-accent hover:border-primary transition-colors w-full flex flex-col">
								<CardHeader>
									<div className="flex items-center gap-4">
										<div className="bg-primary/10 p-3 rounded-lg">
											<action.icon className="h-6 w-6 text-primary" />
										</div>
										<CardTitle className="text-lg">{action.title}</CardTitle>
									</div>
								</CardHeader>
								<CardContent className="flex-grow">
									<p className="text-sm text-muted-foreground">
										{action.description}
									</p>
								</CardContent>
							</Card>
						</Link>
					))}
				</div>
			</div>
		</div>
	);
}

const OverviewStatCard = ({
	title,
	value,
	icon: Icon,
	isLoading,
}: {
	title: string;
	value: string | number;
	icon: React.ElementType;
	isLoading: boolean;
}) => (
	<Card>
		<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
			<CardTitle className="text-sm font-medium">{title}</CardTitle>
			<Icon className="h-4 w-4 text-muted-foreground" />
		</CardHeader>
		<CardContent>
			{isLoading ? (
				<Skeleton className="h-8 w-16" />
			) : (
				<div className="text-2xl font-bold">{value}</div>
			)}
		</CardContent>
	</Card>
);

// --- Token Verification Component (Shared by MessAdmin) ---
const verifySchema = z.object({
	token: z.string().min(1, "Token is required."),
});

function TokenVerificationSection({
	onVerify,
}: {
	onVerify: (data: VerifiedMealData) => void;
}) {
	const { verifyToken } = useData();
	const { toast } = useToast();
	const [isVerifying, setIsVerifying] = useState(false);

	const form = useForm<z.infer<typeof verifySchema>>({
		resolver: zodResolver(verifySchema),
		defaultValues: { token: "" },
	});

	const handleVerifyToken = async (values: z.infer<typeof verifySchema>) => {
		setIsVerifying(true);
		const result = await verifyToken(values.token);
		if (result.success && result.data) {
			toast({
				title: "Token Verified",
				description: "Meal and add-ons marked as consumed.",
				className: "bg-green-100 dark:bg-green-900 border-green-400",
			});
			onVerify(result.data);
		} else {
			toast({
				variant: "destructive",
				title: "Invalid Token",
				description:
					result.error || "This token is invalid or has already been used.",
			});
		}
		form.reset();
		setIsVerifying(false);
	};

	return (
		<Card>
			<CardHeader>
				<CardTitle>Verify Token</CardTitle>
				<CardDescription>
					Enter the student's token to verify their meal.
				</CardDescription>
			</CardHeader>
			<CardContent>
				<Form {...form}>
					<form
						onSubmit={form.handleSubmit(handleVerifyToken)}
						className="flex items-start gap-2"
					>
						<FormField
							control={form.control}
							name="token"
							render={({ field }) => (
								<FormItem className="flex-grow">
									<FormControl>
										<Input
											placeholder="Enter token..."
											{...field}
											className="font-mono uppercase"
										/>
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
						/>
						<Button type="submit" disabled={isVerifying}>
							{isVerifying ? (
								<Loader2 className="h-4 w-4 animate-spin" />
							) : (
								<Search className="h-4 w-4 mr-2" />
							)}
							Verify
						</Button>
					</form>
				</Form>
			</CardContent>
		</Card>
	);
}

function VerifiedMealCard({ data }: { data: VerifiedMealData }) {
	return (
		<Card className="border-primary">
			<CardHeader>
				<CardTitle className="flex items-center gap-2 text-green-600">
					<CheckCircle />
					<span>Token Verified Successfully</span>
				</CardTitle>
				<CardDescription>
					Order details for {data.meal.studentName}
				</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="space-y-4">
					<div>
						<h4 className="font-semibold flex items-center gap-2">
							<Utensils className="h-4 w-4" />
							<span>Meal</span>
						</h4>
						<p className="pl-6 text-muted-foreground">
							{data.meal.quantity}x {data.meal.mealType}
						</p>
					</div>
					{data.addons.length > 0 && (
						<>
							<Separator />
							<div>
								<h4 className="font-semibold">Add-ons</h4>
								<ul className="list-disc pl-10 text-muted-foreground space-y-1 mt-2">
									{data.addons.map((addon, i) => (
										<li key={i}>
											{addon.quantity}x {addon.name}
										</li>
									))}
								</ul>
							</div>
						</>
					)}
				</div>
			</CardContent>
		</Card>
	);
}

function ActiveTokensTable() {
	const { mealSelections, users, addonConsumptions } = useData();
	const [today, setToday] = useState<string | null>(null);

	useEffect(() => {
		setToday(new Date().toISOString().split("T")[0]);
	}, []);

	const activeTokens = today
		? mealSelections.filter((sel) => sel.date === today && !sel.consumed)
		: [];
	const getUserName = (userId: string) =>
		users.find((u) => u.id === userId)?.name || "Unknown";
	const getAddonsForSelection = (selectionId: string) => {
		return addonConsumptions.filter((ac) => ac.mealSelectionId === selectionId);
	};

	return (
		<Card>
			<CardHeader>
				<CardTitle>Today&apos;s Active Tokens</CardTitle>
				<CardDescription>All pending meal tokens for today.</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="rounded-md border">
					<Table>
						<TableHeader>
							<TableRow>
								<TableHead>Student</TableHead>
								<TableHead>Meal</TableHead>
								<TableHead>Qty</TableHead>
								<TableHead>Add-ons</TableHead>
								<TableHead className="text-right">Status</TableHead>
							</TableRow>
						</TableHeader>
						<TableBody>
							{activeTokens.length > 0 ? (
								activeTokens.map((sel) => {
									const addons = getAddonsForSelection(sel.id);
									return (
										<TableRow key={sel.id}>
											<TableCell>{getUserName(sel.userId)}</TableCell>
											<TableCell>{sel.mealType}</TableCell>
											<TableCell className="font-bold text-center">
												{sel.quantity}
											</TableCell>
											<TableCell>
												<div className="flex flex-wrap gap-1">
													{addons.length > 0 ? (
														addons.map((ac) => (
															<Badge key={ac.id} variant="outline">
																{ac.addon?.name || "Item"}
															</Badge>
														))
													) : (
														<span className="text-xs text-muted-foreground">
															None
														</span>
													)}
												</div>
											</TableCell>
											<TableCell className="text-right">
												<Badge variant="secondary">
													<Hourglass className="h-4 w-4 mr-1" />
													Pending
												</Badge>
											</TableCell>
										</TableRow>
									);
								})
							) : (
								<TableRow>
									<TableCell colSpan={5} className="h-24 text-center">
										No active tokens for today.
									</TableCell>
								</TableRow>
							)}
						</TableBody>
					</Table>
				</div>
			</CardContent>
		</Card>
	);
}

// --- Regular Admin Dashboard ---
function AdminDashboard() {
	const { user } = useAuth();
	const [stats, setStats] = useState({ studentCount: 0, messAdminCount: 0 });
	const [isLoadingStats, setIsLoadingStats] = useState(true);

	useEffect(() => {
		async function fetchStats() {
			setIsLoadingStats(true);
			try {
				const res = await fetchApi("/api/stats");
				if (res.ok) {
					const data = await res.json();
					setStats(data);
				} else {
					console.error("Failed to fetch admin stats: ", await res.text());
				}
			} catch (error) {
				console.error("Failed to fetch admin stats", error);
			} finally {
				setIsLoadingStats(false);
			}
		}
		fetchStats();
	}, []);

	const quickActions = [
		{
			title: "Verify Students",
			href: "/admin/verify-students",
			icon: UserCheck,
			description: "Approve new student accounts.",
		},
		{
			title: "Student Details",
			href: "/admin/student-details",
			icon: Users,
			description: "View all approved students.",
		},
		{
			title: "Manage Mess Admins",
			href: "/admin/manage-mess-admins",
			icon: Contact2,
			description: "Manage token verifiers.",
		},
		{
			title: "Send Notifications",
			href: "/admin/notifications",
			icon: Bell,
			description: "Broadcast announcements.",
		},
		{
			title: "View Feedback",
			href: "/admin/feedback",
			icon: MessageSquare,
			description: "Review student feedback.",
		},
	];

	return (
		<div className="flex flex-col gap-8">
			<div className="text-center">
				<h1 className="text-2xl font-bold md:text-3xl lg:text-4xl">
					Admin Dashboard
				</h1>
				<p className="text-muted-foreground">{user?.hostel} Hostel</p>
			</div>

			<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
				<OverviewStatCard
					title="Students in Hostel"
					value={stats.studentCount}
					icon={Users}
					isLoading={isLoadingStats}
				/>
				<OverviewStatCard
					title="Mess Admins in Hostel"
					value={stats.messAdminCount}
					icon={Contact2}
					isLoading={isLoadingStats}
				/>
				<Card>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
						<CardTitle className="text-sm font-medium">
							Daily Operations
						</CardTitle>
						<ShieldCheck className="h-4 w-4 text-muted-foreground" />
					</CardHeader>
					<CardContent>
						<div className="text-2xl font-bold">Mess Admin</div>
						<p className="text-xs text-muted-foreground">
							Token verification is handled by Mess Admins.
						</p>
					</CardContent>
				</Card>
			</div>

			<div>
				<h2 className="text-xl font-bold md:text-2xl mb-4 text-center md:text-left">
					Quick Actions
				</h2>
				<div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
					{quickActions.map((action) => (
						<Link href={action.href} key={action.href} className="flex">
							<Card className="hover:bg-accent hover:border-primary transition-colors w-full flex flex-col">
								<CardHeader>
									<div className="flex items-center gap-4">
										<div className="bg-primary/10 p-3 rounded-lg">
											<action.icon className="h-6 w-6 text-primary" />
										</div>
										<CardTitle className="text-lg">{action.title}</CardTitle>
									</div>
								</CardHeader>
								<CardContent className="flex-grow">
									<p className="text-sm text-muted-foreground">
										{action.description}
									</p>
								</CardContent>
							</Card>
						</Link>
					))}
				</div>
			</div>
		</div>
	);
}

// --- Mess Admin Dashboard ---
function MessAdminDashboard() {
	const { user } = useAuth();
	const [lastVerified, setLastVerified] = useState<VerifiedMealData | null>(
		null
	);

	const quickActions = [
		{
			title: "Edit Menu",
			href: "/admin/menu",
			icon: Settings,
			description: "Update the weekly meal plan.",
		},
		{
			title: "Send Notifications",
			href: "/admin/notifications",
			icon: Bell,
			description: "Broadcast announcements to students.",
		},
		{
			title: "View Feedback",
			href: "/admin/feedback",
			icon: MessageSquare,
			description: "Review all student feedback.",
		},
	];

	return (
		<div className="flex flex-col gap-8">
			<div className="text-center">
				<h1 className="text-2xl font-bold md:text-3xl lg:text-4xl">
					Mess Admin Dashboard
				</h1>
				<p className="text-muted-foreground">{user?.hostel} Hostel</p>
			</div>

			<div className="grid gap-6 md:grid-cols-2">
				<div className="flex flex-col gap-6">
					<TokenVerificationSection onVerify={setLastVerified} />
					{lastVerified && <VerifiedMealCard data={lastVerified} />}
				</div>
				<ActiveTokensTable />
			</div>

			<div>
				<h2 className="text-xl font-bold md:text-2xl mb-4 text-center md:text-left">
					Quick Actions
				</h2>
				<div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
					{quickActions.map((action) => (
						<Link href={action.href} key={action.href} className="flex">
							<Card className="hover:bg-accent hover:border-primary transition-colors w-full flex flex-col">
								<CardHeader>
									<div className="flex items-center gap-4">
										<div className="bg-primary/10 p-3 rounded-lg">
											<action.icon className="h-6 w-6 text-primary" />
										</div>
										<CardTitle className="text-lg">{action.title}</CardTitle>
									</div>
								</CardHeader>
								<CardContent className="flex-grow">
									<p className="text-sm text-muted-foreground">
										{action.description}
									</p>
								</CardContent>
							</Card>
						</Link>
					))}
				</div>
			</div>
		</div>
	);
}
